...
export function day15_part2(rawFileData: string) {
    ...
    for (let y = 0; y < inputGrid.length; y++) {
        for (let x = 0; x < inputGrid[0].length; x++) {
            ...
            if (inputGrid[y][x] === 'E' || inputGrid[y][x] === 'G') {
                initialUnits.push(new Unit(y, x, Type.parseType(inputGrid[y][x])));
    ...
    function combat(attackPowerOfElves: number = 3): number {
        const units: Unit[] = initialUnits.map(u => new Unit(u.y, u.x, u.type));
        let roundCounter = 0;
        ...
        mainLoop : while (true) {
            ...
            for (const unit of aliveUnits()) {
                ...
                if (enemyToAttack) {
                    enemyToAttack.hitPoints -= unit.type === Type.E ? attackPowerOfElves : 3;

                    if (enemyToAttack.type === Type.E && enemyToAttack.hitPoints <= 0) {
                        break mainLoop;
                    }
                }
            }
        ...
        return units.filter(u => u.type === Type.E).length - aliveUnits().filter(u => u.type === Type.E).length !== 0
            ? undefined
            : roundCounter * aliveUnits().map(u => u.hitPoints).reduce((p, c) => p + c, 0);
    }

    let resultOutcome: number;
    for (let i = 4; !resultOutcome; i++) {
        const result = combat(i);
        if (result) {
